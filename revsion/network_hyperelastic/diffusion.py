...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sample(self, shape,self_cond, clip_denoised = True):
    ...
    def p_losses(self, x_start, t, x_self_cond):
    ...
    def forward(self,img,x_self_cond):
        if self.train:
            b, c, h, w, device, img_size, = *img.shape, img.device, self.image_size
            # assert h == img_size and w == img_size, f'height and width of image must be {img_size}'
            t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
            # img = normalize_to_neg_one_to_one(img)
            return self.p_losses(img, t,x_self_cond)

        if not self.train:
            if not self.is_ddim_sampling:
                print("DDPM sampling")
                # return self.p_sample_loop(img.shape, x_self_cond)
                pred=self.p_sample_loop(img, x_self_cond)             
                return pred

            else:
                print("DDIM sampling")
                pred=self.ddim_sample(img, x_self_cond)             
                return pred
    ...
    #         print("DDPM sampling")
    #         def forward(self, img, x_self_cond):
    #             pred=self.p_sample_loop(img.shape, self_cond)             
    #             return pred

    #     if self.is_ddim_sampling:
    #         print("DDIM sampling")
    #         def forward(self, img, x_self_cond):
    #             pred=self.ddim_sample(img.shape, self_cond)             
    #             return pred